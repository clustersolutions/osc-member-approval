  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      if ($error == false) {
        ...
        tep_db_query("update member_approval set status = " . $status . " where customers_id = '" . (int)$customers_id . "'");
        ...
        tep_redirect(tep_href_link('customers_member_approval.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $customers_id));
      ...
      case 'deleteconfirm':
        ...
        tep_db_query("delete from member_approval where customers_id = '" . (int)$customers_id . "'");

        tep_redirect(tep_href_link('customers_member_approval.php', tep_get_all_get_params(array('cID', 'action'))));
        break;
      case 'approveconfirm':
        $customers_id = tep_db_prepare_input($HTTP_GET_VARS['cID']);

        if (isset($HTTP_POST_VARS['email_confirm']) && ($HTTP_POST_VARS['email_confirm'] == 'on')) {
        ...
        tep_db_query("update member_approval set status = 1, updated = now() where customers_id = '" . (int)$customers_id . "'");
      ...
      case 'resetlink':
        ...
        tep_db_query("update member_approval set validation_key_date = now() where customers_id = '" . (int)$customers_id . "'");
      ...
      case 'expirelink':
        ...
        tep_db_query("update member_approval set validation_key_date = date_sub(now(), interval " . MODULE_CONTENT_MEMBER_APPROVAL_LINK_DAY_EXPIRE . " day) where customers_id = '" . (int)$customers_id . "'");

        tep_redirect(tep_href_link('customers_member_approval.php', tep_get_all_get_params(array('cID', 'action'))));
        break;
      default:
        $customers_query = tep_db_query("select c.customers_id, c.customers_gender, c.customers_firstname, c.customers_lastname, c.customers_dob, c.customers_email_address, a.entry_company, a.entry_street_address, a.entry_suburb, a.entry_postcode, a.entry_city, a.entry_state, a.entry_zone_id, a.entry_country_id, c.customers_telephone, c.customers_fax, c.customers_newsletter, c.customers_default_address_id, ma.status from customers c left join address_book a on c.customers_default_address_id = a.address_book_id, member_approval ma where a.customers_id = c.customers_id and c.customers_id = ma.customers_id and c.customers_id = '" . (int)$HTTP_GET_VARS['cID'] . "'");
...
?>
      ...
      <tr><?php echo tep_draw_form('customers', 'customers_member_approval.php', tep_get_all_get_params(array('action')) . 'action=update', 'post', 'onsubmit="return check_form();"') . tep_draw_hidden_field('default_address_id', $cInfo->customers_default_address_id); ?>
      ...
      <tr>
        <td class="formArea"><table border="0" cellspacing="2" cellpadding="2">
          <tr>
            ...
            <td class="main">
<?php 
              echo tep_draw_radio_field('status', '1', false, $cInfo->status) . '&nbsp;' . ACTIVE . '&nbsp;&nbsp;' . tep_draw_radio_field('status', '0', false, $cInfo->status) . '&nbsp;' . INACTIVE . '&nbsp;' . ENTRY_STATUS_ERROR; 
?>
            </td>
          </tr>
...
?></td>
      ...
      <tr>
        <td align="right" class="smallText"><?php echo tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('customers_member_approval.php', tep_get_all_get_params(array('action')))); ?></td>
      </tr></form>
<?php
  } else {
?>
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr><?php echo tep_draw_form('search', 'customers_member_approval.php', '', 'get'); ?>
            <td class="pageHeading"><?php echo HEADING_TITLE; ?></td>
          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                ...
                <td class="dataTableHeadingContent" align="right"><?php echo TABLE_HEADING_ACCOUNT_STATUS; ?></td>
...
<?php
    ...
    if (isset($HTTP_GET_VARS['search']) && tep_not_null($HTTP_GET_VARS['search'])) {
      ...
      $search = "where c.customers_lastname like '%" . $keywords . "%' or c.customers_firstname like '%" . $keywords . "%' or c.customers_email_address like '%" . $keywords . "%' and";
    } else {
      $search = "where";
    }  
    $customers_query_raw = "select c.customers_id, c.customers_lastname, c.customers_firstname, c.customers_email_address, a.entry_country_id, ma.status, ma.validation_key, DATE_FORMAT(date_add(ma.validation_key_date, interval " . MODULE_CONTENT_MEMBER_APPROVAL_LINK_DAY_EXPIRE . " day), '%m-%d-%Y %h:%i %p') as expiration from customers c left join address_book a on c.customers_id = a.customers_id and c.customers_default_address_id = a.address_book_id, member_approval ma " . $search . " ma.customers_id = c.customers_id order by ma.status, c.customers_id desc";
    ...
    while ($customers = tep_db_fetch_array($customers_query)) {
      ...
      if (isset($cInfo) && is_object($cInfo) && ($customers['customers_id'] == $cInfo->customers_id)) {
        echo '          <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link('customers_member_approval.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=edit') . '\'">' . "\n";
      } else {
        echo '          <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link('customers_member_approval.php', tep_get_all_get_params(array('cID')) . 'cID=' . $customers['customers_id']) . '\'">' . "\n";
      }
?>
...
?>
                ...
                <td class="dataTableContent" align="right"><?php if (isset($cInfo) && is_object($cInfo) && ($customers['customers_id'] == $cInfo->customers_id)) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . tep_href_link('customers_member_approval.php', tep_get_all_get_params(array('cID')) . 'cID=' . $customers['customers_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
?>
...
?>
                  <tr>
                    <td class="smallText" align="right" colspan="2"><?php echo tep_draw_button(IMAGE_RESET, 'arrowrefresh-1-w', tep_href_link('customers_member_approval.php')); ?></td>
                  </tr>
<?php
    }
?>
...
<?php
  ...
  switch ($action) {
    case 'confirm':
      ...
      $contents = array('form' => tep_draw_form('customers', 'customers_member_approval.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=deleteconfirm'));
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_DELETE, 'trash', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('customers_member_approval.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id)));
      break;
    case 'approve':
      $heading[] = array('text' => '<strong>' . TEXT_INFO_HEADING_APPROVE_CUSTOMER . '</strong>');

      $contents = array('form' => tep_draw_form('customers', 'customers_member_approval.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=approveconfirm'));
      $contents[] = array('text' => TEXT_APPROVE_INTRO . '<br /><br /><strong>' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '</strong>');
      $contents[] = array('text' => '<br />' . tep_draw_checkbox_field('email_confirm', 'on', true) . ' ' . TEXT_EMAIL_CONFIRM);
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_CONTINUE, 'carat-1-e', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('customers_member_approval.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id)));
      break;
    case 'reset':
      $heading[] = array('text' => '<strong>' . TEXT_INFO_HEADING_RESET_LINK . '</strong>');

      $contents = array('form' => tep_draw_form('customers', 'customers_member_approval.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=resetlink'));
      $contents[] = array('text' => TEXT_RESET_LINK . '<br /><br /><strong>' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '</strong>');
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_CONTINUE, 'carat-1-e', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('customers_member_approval.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id)));
      break;
    case 'expire':
      $heading[] = array('text' => '<strong>' . TEXT_INFO_HEADING_EXPIRE_LINK . '</strong>');

      $contents = array('form' => tep_draw_form('customers', 'customers_member_approval.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=expirelink'));
      $contents[] = array('text' => TEXT_EXPIRE_LINK . '<br /><br /><strong>' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '</strong>');
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_CONTINUE, 'carat-1-e', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('customers_member_approval.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id)));
      break;
    default:
      if (isset($cInfo) && is_object($cInfo)) {
        ...
        $contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_EDIT, 'document', tep_href_link('customers_member_approval.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=edit')) . tep_draw_button(IMAGE_DELETE, 'trash', tep_href_link('customers_member_approval.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=confirm')) . tep_draw_button(IMAGE_ORDERS, 'cart', tep_href_link(FILENAME_ORDERS, 'cID=' . $cInfo->customers_id)) . tep_draw_button(IMAGE_EMAIL, 'mail-closed', tep_href_link(FILENAME_MAIL, 'customer=' . $cInfo->customers_email_address)));
        $contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_APPROVE_CONFIRM, 'check', tep_href_link('customers_member_approval.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=approve')) . tep_draw_button(IMAGE_RESET_LINK, 'arrowrefresh-1-w', tep_href_link('customers_member_approval.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=reset')) . tep_draw_button(IMAGE_EXPIRE_LINK, 'circle-close', tep_href_link('customers_member_approval.php', tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=expire')));
        ...
        $contents[] = array('text' => '<br />' . ENTRY_STATUS . ': ' . ($cInfo->status ? ACTIVE : INACTIVE));
        $contents[] = array('text' => '<br />' . TEXT_DOCUMENT_LINK . ' <a href="' . HTTPS_SERVER . DIR_WS_HTTPS_CATALOG . 'registration_upload.php?key=' . $cInfo->validation_key . '" target="_blank">' . TEXT_LINK . '</a>&nbsp;' . TEXT_EXPIRE . ' ' . $cInfo->expiration);
      }
      break;
  }
